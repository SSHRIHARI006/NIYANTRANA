...
function AddTrainWithTypedCsvUpload() {
  ...
  const sendTrain = async (train) => {
    try {
      const response = await fetch('http://127.0.0.1:4001/api/addtrain', { // Replace with your API URL
        method: 'POST',
        headers: {
...
export default function AddTrain() {
  ...
  const handleSubmit = (e) => {
    ...
    const payload = {
        train_id: parseInt(form.train_id, 10),
        is_fit_for_service: form.fit_status=='fit',
        mileage_kms_this_month: parseFloat(form.mileage_kms_this_month),
        has_branding: form.has_branding,
        branding_days_completed: form.has_branding ? parseInt(form.branding_days_completed, 10) : 0,
        branding_days_required: form.has_branding ? parseInt(form.branding_days_required, 10) : 0,
